...
supports a network connection. Look for "fitting the firmware to your hardware device" in the README. If you aren't too afraid of jumping into the firmware, there is also support for configuring the boards "virtual pins" struct, which allows you to change the functionality of the firmware when the Pulse receives a "virtual PWM" over mqtt.
...
**Writing values to the Pulse over mqtt**
